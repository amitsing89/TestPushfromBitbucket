...
sessionadlog.default_timeout = 30

# Csv File Location
f2 = open('/var/www/html/conversion/attribution' + sys.argv[1] + sys.argv[2] + sys.argv[3] + ".csv", 'w')
csv_file2 = csv.writer(f2)
csv_file2.writerow(["AdType", "Clientid", "AdlogType", "Userid", "GeoId", "GoalId", "ItemId", "jsts", "Lineitemid", "V",
                    "EarlyAttribution", "IsAttributed", "Datehourmin", "Mod", "CreatedTime"])

# Csv File Location
f1 = open('/var/www/html/conversion/bilattribution' + sys.argv[1] + sys.argv[2] + sys.argv[3] + ".csv", 'w')
csv_file = csv.writer(f1)
csv_file.writerow(
    ["Date", "AdvertiserId", "PublisherId", "LineitemId", "ItemId", "AdslotId", "GoalId", "SiteId", "SectionId",
     "GeoId", "Spend", "EarlyAttribution", "DatehourMin", "Mod", "UserId"])
...
def algoBilAttributionDetails(year, month, datehourminlist, adv):
    ...
    for datehourminvar in datehourminlist:
        query = "select * from wls.ba{0}{1} where clientid = {2} and datehourmin = {3} and mod in {4}".format(year,
                                                                                                              month,
                                                                                                              adv, int(
                datehourminvar), tuple(mod))
        logval = "select * from wls.ba{0}{1} where clientid = {2} and datehourmin = {3}".format(year, month, adv,
                                                                                                int(datehourminvar))
        logging.info(logval)
        try:
            ...
            for a in rows:
                ...
                csv_file.writerow(
                    [convertedDate, a.clientid, a.pubclientid, a.itemcolumbiaid, a.itemid, a.adslotdimid, a.goalid,
                     a.siteid, a.section, a.geodimid, a.spend, a.earlyattribution, a.datehourmin, a.mod, a.clmbuserid])
...
def algoAttributionDetails(year, month, datehourminlist, adv):
    ...
    for num in range(0, 1000):
        mod.append(int(num))
    for datehourminvar in datehourminlist:
        query = "select * from adtracker{0}{1} where clientid = {2} and datehourmin = {3} and mod in {4}".format(year,
                                                                                                                 month,
                                                                                                                 adv,
                                                                                                                 int(
                                                                                                                     datehourminvar),
                                                                                                                 tuple(
                                                                                                                     mod))
        logval = "select * from adtracker{0}{1} where clientid = {2} and datehourmin = {3}".format(year, month, adv,
                                                                                                   int(datehourminvar))
        logging.info(logval)
        try:
            ...
            for user_row in rows:
                ...
                osDetails = ""
                modval = user_row.mod
                datehourminvar = user_row.datehourmin
                ...
                if 'UnKnown, More-Info: Apsalar-Postback' in osDetails:
                    if isAtributed is True:
                        csv_file2.writerow(
                            ["Apsalar True", user_row.clientid, user_row.adlogtype, user_row.userid, user_row.geoid,
                             user_row.goalid, user_row.itemid, user_row.jsts, user_row.lineitemid, user_row.v,
                             user_row.earlyattribution, user_row.isattributed, user_row.datehourmin, user_row.mod,
                             user_row.createtime])
                    elif isAtributed is not True:
                        csv_file2.writerow(
                            ["Apsalar Failure", user_row.clientid, user_row.adlogtype, user_row.userid, user_row.geoid,
                             user_row.goalid, user_row.itemid, user_row.jsts, user_row.lineitemid, user_row.v,
                             user_row.earlyattribution, user_row.isattributed, user_row.datehourmin, user_row.mod,
                             user_row.createtime])
                    ...
                    if isAtributed is True:
                        csv_file2.writerow(
                            ["Mat True", user_row.clientid, user_row.adlogtype, user_row.userid, user_row.geoid,
                             user_row.goalid, user_row.itemid, user_row.jsts, user_row.lineitemid, user_row.v,
                             user_row.earlyattribution, user_row.isattributed, user_row.datehourmin, user_row.mod,
                             user_row.createtime])
                    elif isAtributed is not True:
                        csv_file2.writerow(
                            ["Mat Failure", user_row.clientid, user_row.adlogtype, user_row.userid, user_row.geoid,
                             user_row.goalid, user_row.itemid, user_row.jsts, user_row.lineitemid, user_row.v,
                             user_row.earlyattribution, user_row.isattributed, user_row.datehourmin, user_row.mod,
                             user_row.createtime])
                else:
                    if isAtributed is True:
                        csv_file2.writerow(
                            ["Web True", user_row.clientid, user_row.adlogtype, user_row.userid, user_row.geoid,
                             user_row.goalid, user_row.itemid, user_row.jsts, user_row.lineitemid, user_row.v,
                             user_row.earlyattribution, user_row.isattributed, user_row.datehourmin, user_row.mod,
                             user_row.createtime])
                    elif isAtributed is not True:
                        csv_file2.writerow(
                            ["Web Failure", user_row.clientid, user_row.adlogtype, user_row.userid, user_row.geoid,
                             user_row.goalid, user_row.itemid, user_row.jsts, user_row.lineitemid, user_row.v,
                             user_row.earlyattribution, user_row.isattributed, user_row.datehourmin, user_row.mod,
                             user_row.createtime])
...
try:
    for adv in advclientids:
        ...
        threadalgoattribution = threading.Thread(target=algoAttributionDetails,
                                                 kwargs={'year': sys.argv[1], 'month': sys.argv[2],
                                                         'datehourminlist': datehourminlist, 'adv': adv})
        threadalgoattribution.start()
except Exception as e:
    logging.error(e)
    me = "asheesh.mahor@timesinternet.in"
    to = ["asheesh.mahor@timesinternet.in", "amit.singh2@timesinternet.in"]
...
try:
    for adv in advclientids:
        datehourminlist = datehourminlistCreator(sys.argv[1], sys.argv[2], sys.argv[3])
        threadalgoBilAttributionDetails = threading.Thread(target=algoBilAttributionDetails,
                                                           kwargs={'year': sys.argv[1], 'month': sys.argv[2],
                                                                   'datehourminlist': datehourminlist, 'adv': adv})
    ...
    logging.error(e)
    me = "asheesh.mahor@timesinternet.in"
    to = ["amit.singh2@timesinternet.in", "amit.singh2@timesinternet.in"]
    # to = ["asheesh.mahor@timesinternet.in","saurabh.chandolia@timesinternet.in","amit.singh2@timesinternet.in","kundan.kumar1@timesinternet.in","gaurav.sharma8@timesinternet.in","mohit.kundra@timesinternet.in","varun.arora1@timesinternet.in","vijendra.dewda@timesinternet.in","mankeshwar.kumar@timesinternet.in"]
    msg = MIMEMultipart('alternative')
    msg['Subject'] = "Exception in Postbackfailure"
    msg['From'] = me
