...
import modelforms.RedisInsertion as redis
...
class IndexView(generic.ListView):
    ...
    # w.writerow(['product_title', 'product_price', 'product_desc'])

    def get_queryset(self):
        y = Product.objects.all()
        if y.exists():
            for i in y.values().latest('id'):
                if 'id' not in i:
                    redis.insertMetadata(i, y.values().latest('id')[i])
