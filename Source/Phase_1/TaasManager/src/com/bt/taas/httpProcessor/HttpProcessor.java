package com.bt.taas.httpProcessor;

import static com.bt.taas.constants.Constant.*;
...
import com.bt.taas.httpProperties.HttpProperties;
...
public class HttpProcessor {
	...
	public void setProperties(String configfilepath) {
		...
		try {
			...
			/*if ("".equalsIgnoreCase(jksfilepath)) {
				throw new Error("Jksfilepath cannot be blank");
			} else
	 ...
	 * @throws IOException
	 */
	public String getAuthKey(String jksfile) throws ClientProtocolException, IOException {
		...
		HttpEntity entity = MultipartEntityBuilder.create().addTextBody("username",tokenization.getUsername()).addBinaryBody("password",
				new File(jksfile), ContentType.create("application/octet-stream"),jksfile).build();
	...
	/*
	 * public String getToken(String authtoken,String number) throws
	 * ClientProtocolException, IOException{
	 * 
	 * //System.out.println("inside getToken"+ authtoken); String service =
	 * "api/getToken"; String url = "http://" + tokenization.getHost() + ":" +
	 * tokenization.port + "/" + service; HttpClient client =
	 * HttpClientBuilder.create().build(); HttpPost post = new HttpPost(url);
	 * 
	 * post.setHeader("Content-Type","application/json");
	 * post.setHeader("x-access-token",authtoken);
	 * 
	 * List<NameValuePair> urlParameters = new ArrayList<NameValuePair>();
	 * //urlParameters.add(new
	 * BasicNameValuePair("content-type","application/json"));
	 * //urlParameters.add(new BasicNameValuePair("x-access-token",authtoken));
	 * urlParameters.add(new BasicNameValuePair("input",number)); //
	 * "multipart/form-data; boundary=---011000010111000001101001")); //
	 * //System.out.println("url params =>" + new
	 * UrlEncodedFormEntity(urlParameters).); post.setEntity(new
	 * UrlEncodedFormEntity(urlParameters)); HttpResponse response =
	 * client.execute(post); System.out.println("Response Code : " +
	 * response.getStatusLine());
	 * 
	 * BufferedReader rd = new BufferedReader(new
	 * InputStreamReader(response.getEntity().getContent()));
	 * 
	 * StringBuffer result = new StringBuffer(); String line = ""; String
	 * token=""; while ((line = rd.readLine()) != null) { result.append(line);
	 * if(line.contains("token")){
	 * token=line.split(":")[1].replaceAll("\"","").replace("}",""); } }
	 * 
	...
	public static void main(String[] args) throws ClientProtocolException, IOException {
		HttpProcessor tokenret = new HttpProcessor();
		String filepath = "C:\\Users\\609504664\\workspace\\TaaSManager\\configuration.properties";
		tokenret.setProperties(filepath);
		String token = tokenret.getAuthKey(filepath );
		System.out.println("Token generated " + token);
		// System.out.println(tokenret.getToken(token,"9739042229"));
		System.out.println(tokenret.getTokenizeValue(token, "973904256734","2",""));
